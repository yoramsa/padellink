...
const LEVELS = [
  { val: 1.0, label: 'Débutant', labelEn: 'Beginner', color: '#6b7280', desc: 'Tu découvres le padel. Tu apprends à tenir la raquette, les règles de base et à frapper la balle.' },
  { val: 1.5, label: 'Débutant+', labelEn: 'Beginner+', color: '#9ca3af', desc: 'Tu connais les règles. Tu arrives à faire quelques échanges et tu commences à viser.' },
  { val: 2.0, label: 'Intermédiaire', labelEn: 'Intermediate', color: '#3b82f6', desc: 'Tu es régulier dans tes frappes. Les coups de base (coup droit, revers) sont maîtrisés.' },
  { val: 2.5, label: 'Intermédiaire+', labelEn: 'Intermediate+', color: '#06b6d4', desc: 'Tu lis bien le jeu, tu construis des points et tu utilises une tactique simple.' },
  { val: 3.0, label: 'Confirmé', labelEn: 'Confirmed', color: '#10b981', desc: 'Tu maîtrises le jeu au mur, tu varies tes frappes et tu sais jouer en équipe.' },
  { val: 3.5, label: 'Confirmé+', labelEn: 'Confirmed+', color: '#84cc16', desc: 'Tu exécutes des bandejas et viboras. Ton jeu est complet et tu participes à des tournois.' },
  { val: 4.0, label: 'Avancé', labelEn: 'Advanced', color: '#f59e0b', desc: 'Tu joues des tournois régionaux avec constance. Tu as une vraie tactique de jeu.' },
  { val: 4.5, label: 'Expert', labelEn: 'Expert', color: '#f97316', desc: 'Niveau compétitif national. Tu joues des tournois officiels et tu gagnes régulièrement.' },
  { val: 5.0, label: 'Pro', labelEn: 'Pro', color: '#ef4444', desc: 'Niveau professionnel ou semi-professionnel. Tu es classé nationalement ou internationalement.' },
]
...
const STYLES = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .logo{font-family:'Bebas Neue',cursive;font-size:40px;letter-spacing:4px;background:linear-gradient(135deg,#a855f7,#06b6d4);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:4px;}
  .sub{font-size:12px;color:#6b7280;margin-bottom:32px;letter-spacing:1px;}
...
export default function CreateProfile({ session, onCreated }) {
  ...
  async function handleSubmit() {
    if (!name.trim() || name.trim().length < 2) {
      setError('Entre ton prénom et nom (min. 2 caractères).')
      return
    }
    if (!city.trim() || city.trim().length < 2) {
      setError('Entre ta ville.')
      return
    }
    if (!dob) {
  ...
  return (
    <div>
      ...
      <div className="wrap">
        ...
        <div className="sub">CRÉE TON PROFIL JOUEUR</div>
        <div className="card">
          <div className="step">✨ Première connexion</div>
          <div className="title">Bienvenue !</div>
          <div className="desc">
            Configure ton profil pour rejoindre la communauté padel.
          </div>

          <div className="label">Prénom et nom</div>
          <input
            className="input"
          ...
          <div className="label">Niveau de jeu</div>
          <select
            className="select"
            value={level}
          ...
          >
            <div style={{
              ...
              {selectedLevel.label} — Niveau {level}
            </div>
