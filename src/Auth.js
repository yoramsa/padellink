...
const STYLES = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .tab{flex:1;padding:8px;border-radius:10px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:#6b7280;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;}
  .tab.active{background:rgba(139,92,246,0.15);border-color:rgba(139,92,246,0.4);color:#a855f7;}
`

export default function Auth() {
  var [mode, setMode] = useState('magic')
  ...
  return React.createElement('div', null,
    ...
    React.createElement('div', { className: 'auth-wrap' },
      React.createElement('div', { className: 'auth-logo' }, 'PadelLink'),
      React.createElement('div', { className: 'auth-sub' }, 'LA COMMUNAUTÉ PADEL EN ISRAËL'),
      React.createElement('div', { className: 'auth-card' },
        React.createElement('div', { className: 'auth-title' }, '👋 Connexion'),
        React.createElement('div', { className: 'auth-desc' },
          'Entre ton email et reçois un lien magique — pas de mot de passe à retenir.'
        ),
        name === '' && mode === 'magic' ? null :
        ...
        message && React.createElement('div', {
          className: 'msg ' + (isError ? 'msg-error' : 'msg-success')
        }, message)
      ),
      React.createElement('div', { style: { marginTop: 24, fontSize: 11, color: '#374151', textAlign: 'center' } },
